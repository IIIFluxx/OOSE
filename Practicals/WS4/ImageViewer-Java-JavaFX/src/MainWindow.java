...
public class MainWindow
{
    ...
    public void show()
    {
        ...
        // *** Fix this code so that it loads the initial (first) image. *** --- DONE
        //String url = new File("[Initial image filename]").toURI().toString();
        String url = new File(album.getRecordFileNameAtIndex(current)).toURI().toString();
        ...
        // *** Fix this code so that it displays the caption for the first image. *** --- DONE
        //captionWidget.setText("[Initial image caption]");
        captionWidget.setText(album.getInfoAtIndex(current));
    ...
    */
    ...
    {
        // *** Fix this code so that it actually displays the previous image & caption. *** -- DONE

        current = album.iterateBack(current);
        String url = new File(album.getRecordFileNameAtIndex(current)).toURI().toString();
        imageWidget.setImage(new Image(url));
        //captionWidget.setText("[Initial image caption]");
        captionWidget.setText(album.getInfoAtIndex(current));
    ...
    {
        // *** Fix this code so that it actually displays the next image & caption. *** -- DONE
        current = album.iterateForward(current);
        //String url = new File("[Initial image filename]").toURI().toString();
        String url = new File(album.getRecordFileNameAtIndex(current)).toURI().toString();
        imageWidget.setImage(new Image(url));
        //captionWidget.setText("[Initial image caption]");
